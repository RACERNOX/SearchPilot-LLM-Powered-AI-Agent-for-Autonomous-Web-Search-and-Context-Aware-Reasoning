...
def chat_with_groq(messages, stream=False):
    """Chat with Groq API (Free Llama models)"""
    try:
        ...
        while not context_found and len(search_results) > 0:
            ...
            try:
                if best_result < len(search_results):
                    ...
                    page_link = search_results[0]['link']
                    best_result = 0
                context_found = True
            except:
                if search_results:
...
async def generate_response_stream(prompt: str):
    ...
    if not os.getenv("GROQ_API_KEY"):
        yield f"data: {json.dumps({'content': 'Groq API key not configured. Please set GROQ_API_KEY environment variable. Get free key from: https://console.groq.com'})}\n\n"
    ...
    # Stream response
    try:
        # Simple non-streaming response for now (Groq streaming is more complex)
        response = chat_with_groq(assistant_convo, stream=False)
...
async def health_check():
    ...
    return {"status": "healthy", "service": "SearchPilot", "api": "Groq"}
